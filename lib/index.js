...
function filterFileNames(files, options) {
  ...
  const matchesAnyRegex  = function (file, regexList) {
    return regexList.some(regex => regex.test(file));
  };

  return files.filter(file => {
    const isIncluded = include.length === 0 || matchesAnyRegex(file, include);
    const isExcluded = matchesAnyRegex(file, exclude);
...
async function obfuscateBundle(
  ...
  const filteredFileNames = filterFileNames(fileNames, runConfig);

  // Loop through filesSrc and obfuscate files and save to filesDest.
  await obfuscate({ config, filesSrc, filesDest, cwd, filteredFileNames, runConfig});
...
module.exports = function (_config = {}, runConfig = {}, projectRoot = process.cwd()) {
  ...
  process.on('beforeExit', async function (exitCode) {
    try{
      ...
      // Resolve the bundle path here (inside beforeExit) so that for Expo
      // projects the bundle file already exists on disk by the time we look
      // for it.  For bare RN the CLI args are available from the very start,
      // so deferring is safe for both toolchains.
